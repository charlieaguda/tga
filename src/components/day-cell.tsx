...
const EMPTY_FILE_GROUPS: CategoryUploadGroup[] = [];
...
export function DayCell({
  ...
  const dayFileGroups = fileGroups ?? EMPTY_FILE_GROUPS;
  const hasTasks = dayTasks.initiated.length > 0 || dayTasks.due.length > 0;
  const interactive = dayFileGroups.length > 0 || hasTasks;
  ...
  return (
    <div className="relative">
      ...
      >
        {day}
        {(dayTasks.initiated.length > 0 || dayTasks.due.length > 0 || (dayFileGroups.length > 0 && !active)) && (
      ...
      {hovered && interactive && !open && (
        <div className="absolute left-1/2 top-full z-20 mt-1 -translate-x-1/2">
          <DayHoverCard dateLabel={dateLabel} fileGroups={dayFileGroups} tasks={dayTasks} />
        </div>
      )}
