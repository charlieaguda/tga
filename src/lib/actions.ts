...
export async function clientSetDefaultManager(_prev: ActionResult, formData: FormData) {
  const parsed = z
    .object({ clientId: id, defaultManagerId: z.string().trim().optional() })
    .safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { ok: false, error: parsed.error.issues[0].message };
  return guard(() =>
    clients.setClientDefaultManager(parsed.data.clientId, parsed.data.defaultManagerId || null),
  );
}

export async function clientSetDefaultEditor(_prev: ActionResult, formData: FormData) {
  const parsed = z
    .object({ clientId: id, defaultEditorId: z.string().trim().optional() })
    .safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { ok: false, error: parsed.error.issues[0].message };
  return guard(() =>
    clients.setClientDefaultEditor(parsed.data.clientId, parsed.data.defaultEditorId || null),
  );
}
